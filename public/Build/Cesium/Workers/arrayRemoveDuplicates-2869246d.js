 ...
 */
define(['exports', './when-8d13db60', './Check-70bec281', './Math-61ede240'], function (exports, when, Check, _Math) {
    ...
    var removeDuplicatesEpsilon = _Math.CesiumMath.EPSILON10;
         ...
         * @private
         */
        function arrayRemoveDuplicates(values, equalsEpsilon, wrapAround, epsilon) {
            ...
            epsilon = when.defaultValue(epsilon, removeDuplicatesEpsilon);
            ...
            for (i = 1; i < length; ++i) {
                v0 = values[i - 1];
                v1 = values[i];
                if (equalsEpsilon(v0, v1, epsilon)) {
                    break;
                }
            }

            if (i === length) {
                if (wrapAround && equalsEpsilon(values[0], values[values.length - 1], epsilon)) {
            ...
            if (wrapAround && cleanedvalues.length > 1 && equalsEpsilon(cleanedvalues[0], cleanedvalues[cleanedvalues.length - 1], epsilon)) {
                cleanedvalues.shift();
            }
